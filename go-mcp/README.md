...
[MCP](https://modelcontextprotocol.io) server exposing the DigimonTcg SDK as
two agent tools — `digimon-tcg_list` and `digimon-tcg_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/digimon-tcg-mcp)
make build

# 2. Provide credentials via the environment
export DIGIMON_TCG_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user digimon-tcg \
  -- /absolute/path/to/digimon-tcg-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./digimon-tcg-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// digimon-tcg_list: first page of records
{ "entity": "get_all_card" }
{ "entity": "get_all_card", "query": { } }

// digimon-tcg_load: one record by id
{ "entity": "get_all_card", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/digimon-tcg-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user digimon-tcg \
     -- "$PWD"/dist/*/digimon-tcg-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `digimon-tcg_list` and `digimon-tcg_load` tools now appear
   in new sessions. Ask the agent to *"list get_all_card using digimon-tcg"*
   and it calls `digimon-tcg_list` with `{"entity":"get_all_card"}`.
...
```sh
export DIGIMON_TCG_APIKEY=sk_live_xxx            # API key
export DIGIMON_TCG_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `digimon-tcg_list` tool
...
### Call the `digimon-tcg_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `digimon-tcg_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `digimon-tcg_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DIGIMON_TCG_APIKEY` | API key sent with every request. |
| `DIGIMON_TCG_BASE` | Optional override of the API base URL. |
...
```sh
./digimon-tcg-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"digimon-tcg_load","arguments":{"entity":"get_all_card","query":{"id":1}}}}'
```
