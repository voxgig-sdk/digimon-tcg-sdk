...
class TestGetAllCardDirect:

    def test_should_direct_list_get_all_card(self):
        setup = _get_all_card_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-get_all_card", "live" if setup["live"] else "unit")
        if _skip:
...
def _get_all_card_direct_setup(mockres):
