...
class DigimonTcgTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DigimonTcgTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DigimonTcgTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DigimonTcgTestRunner.getenv("DIGIMON_TCG_TEST_LIVE")
        override = DigimonTcgTestRunner.getenv("DIGIMON_TCG_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DigimonTcgTestRunner.getenv(key)
        ...
        explain = DigimonTcgTestRunner.getenv("DIGIMON_TCG_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DIGIMON_TCG_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DigimonTcgTestRunner._test_control is not None:
            return DigimonTcgTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DigimonTcgTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DigimonTcgTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DigimonTcgTestRunner.load_test_control()
...
def load_env_local():
    DigimonTcgTestRunner.load_env_local()


def env_override(m):
    return DigimonTcgTestRunner.env_override(m)


def entity_data(v):
    return DigimonTcgTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DigimonTcgTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DigimonTcgTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DigimonTcgTestRunner.load_test_control()


def live_client_options():
    return DigimonTcgTestRunner.live_client_options()


def live_delay_ms():
    return DigimonTcgTestRunner.live_delay_ms()
