...
class GetAllCardDirectTest < Minitest::Test
  def test_direct_list_get_all_card
    setup = get_all_card_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-get_all_card", setup[:live] ? "live" : "unit")
    if _should_skip
...
def get_all_card_direct_setup(mockres)
