...
module DigimonTcgUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DigimonTcgResponse.new({})
    ...
    if fetch_err
      ...
      response = DigimonTcgResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DigimonTcgResponse.new(fetched)
    else
