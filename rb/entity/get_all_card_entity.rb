...
class GetAllCardEntity
  ...
  def make
    opts = @_entopts.dup
    GetAllCardEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DigimonTcgHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetAllCard, Hash] the current GetAllCard data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DigimonTcgHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetAllCard fields)
  def match_get
  ...
  #
  # @param reqmatch [GetAllCardListMatch, Hash, nil] match filter (any subset of
  #   GetAllCard fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<GetAllCard>, Array] the matching GetAllCard items; raises DigimonTcgError on failure
