...
boru-driven command-line client **and** interactive REPL for the DigimonTcg
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/digimon-tcg-cli)
make build

# 2. See usage (words, entities, env vars)
./digimon-tcg-cli --help

# 3. Provide credentials once, via the environment
export DIGIMON_TCG_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./digimon-tcg-cli list get_all_card
./digimon-tcg-cli list search

# 5. Override the API base URL for a single call
DIGIMON_TCG_BASE=https://api.example.com ./digimon-tcg-cli list get_all_card

# 6. No arguments -> interactive REPL
./digimon-tcg-cli
digimon-tcg> list get_all_card
digimon-tcg> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/digimon-tcg-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/digimon-tcg-cli list get_all_card
   ```
...
Run with no arguments to open a REPL (prompt `digimon-tcg>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_all_card`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/digimon-tcg-cli`. |
...
(`./digimon-tcg-cli <expr>`) and interactively (the REPL), and expressions compose the
