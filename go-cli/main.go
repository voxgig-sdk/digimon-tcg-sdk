// Command DigimonTcg-cli is a CLI + REPL for the DigimonTcg SDK,
...
const prompt = "digimon-tcg"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_all_card search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DIGIMON_TCG_APIKEY (api key), DIGIMON_TCG_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DIGIMON_TCG_APIKEY carries the API key and
	// DIGIMON_TCG_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DIGIMON_TCG_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DIGIMON_TCG_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDigimonTcgSDK(opts)
