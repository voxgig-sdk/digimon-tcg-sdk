...
class DigimonTcgRetryFeature extends DigimonTcgBaseFeature
{
    ...
    public function init(DigimonTcgContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (DigimonTcgContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(DigimonTcgContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(DigimonTcgContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
