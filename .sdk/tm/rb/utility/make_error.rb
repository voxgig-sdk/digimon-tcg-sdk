...
module DigimonTcgUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = DigimonTcgContext.new({}, nil)
    end
    op = ctx.op || DigimonTcgOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || DigimonTcgResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(DigimonTcgError) ? err.msg : err.to_s
    ...
    sdk_err = DigimonTcgError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(DigimonTcgError)
