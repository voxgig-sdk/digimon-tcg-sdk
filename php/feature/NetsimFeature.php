...
class DigimonTcgNetsimFeature extends DigimonTcgBaseFeature
{
    ...
    private function _simulate(DigimonTcgContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(DigimonTcgContext $ctx, array $applied): void
    {
        $client = $this->client;
