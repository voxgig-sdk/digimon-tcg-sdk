...
class GetAllCardEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = DigimonTcgSDK::test(null, null);
        $ent = $testsdk->GetAllCard(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = DigimonTcgSDK::test($seed, null);
        $seen = iterator_to_array($base->GetAllCard(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->GetAllCard(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = get_all_card_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "get_all_card." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $get_all_card_ref01_data = null;
        if (count($get_all_card_ref01_data_raw) > 0) {
...
function get_all_card_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/get_all_card/GetAllCardTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["get_all_card01", "get_all_card02", "get_all_card03"] as $k) {
