...
class FtCtrl extends DigimonTcgControl
{
...
class FtClient
{
    ...
    private DigimonTcgUtility $utility;

    public function __construct(DigimonTcgUtility $utility, array $options)
    {
    ...
    public function get_utility(): DigimonTcgUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DigimonTcgContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DigimonTcgContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DigimonTcgContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DigimonTcgConfig::make_feature($name);
    ...
    private static function buildUrl(DigimonTcgSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DigimonTcgError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DigimonTcgResponse($fetched);
        }
    ...
    private function fail(DigimonTcgContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DigimonTcgContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
