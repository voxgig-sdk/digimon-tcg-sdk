...
// DigimonTcg SDK GetAllCard entity
...
class GetAllCardEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetAllCardEntity($this->_client, $opts);
    }

    /**
     * @param GetAllCard|array $args GetAllCard data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = DigimonTcgHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetAllCard|array The current GetAllCard data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetAllCard fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = DigimonTcgHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetAllCard fields).
     */
    ...
    /**
     * List GetAllCard items matching the given filter.
     *
     * @param GetAllCardListMatch|array|null $reqmatch Match filter (any subset
     *   of GetAllCard fields) as an assoc-array; GetAllCardListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetAllCard[]|array A list of GetAllCard items as assoc-arrays at
     *   the SDK boundary; throws DigimonTcgError on failure (item-5 convention).
     */
