...
The Lua SDK for the DigimonTcg API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:GetAllCard()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local get_all_card, err = client:GetAllCard():list()
    if err then error(err) end
    -- get_all_card is the record list
...
```
lua/
├── digimon-tcg_sdk.lua    -- Main SDK module
...
The main module (`digimon-tcg_sdk`) exports the SDK constructor
