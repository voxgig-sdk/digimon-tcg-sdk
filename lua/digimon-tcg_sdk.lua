...
local DigimonTcgSDK = {}
DigimonTcgSDK.__index = DigimonTcgSDK
...
DigimonTcgSDK._make_feature = _make_feature


function DigimonTcgSDK.new(options)
  local self = setmetatable({}, DigimonTcgSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DigimonTcgSDK:options_map()
...
function DigimonTcgSDK:get_utility()
  return Utility.copy(self._utility)
end


function DigimonTcgSDK:get_root_ctx()
  return self._rootctx
end


function DigimonTcgSDK:prepare(fetchargs)
...
function DigimonTcgSDK:direct(fetchargs)
...
-- by passing it.
function DigimonTcgSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DigimonTcgSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DigimonTcgSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:GetAllCard():list() / client:GetAllCard():load({ id = ... })
...
function DigimonTcgSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DigimonTcgSDK.new(sdkopts)
  sdk.mode = "test"
