...
import (
	...
	omni "github.com/voxgig-sdk/digimon-tcg-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DigimonTcgError); is {
				out := map[string]any{
					"name":    "DigimonTcgError",
...
func omniCtx(arg any, client *sdk.DigimonTcgSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DigimonTcgSDK); is {
			client = live
