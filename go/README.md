...
The Golang SDK for the DigimonTcg API. Provides an entity-oriented interface using standard Go conventions — no generics required, data flows as `map[string]any`.


## Install
```bash
go get github.com/voxgig-sdk/digimon-tcg-sdk
```
...
```bash
go mod edit -replace github.com/voxgig-sdk/digimon-tcg-sdk=../path/to/github.com/voxgig-sdk/digimon-tcg-sdk
```
...
```go
    result, err := client.GetAllCard(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
...
```
github.com/voxgig-sdk/digimon-tcg-sdk/
...
The root package (`github.com/voxgig-sdk/digimon-tcg-sdk`) re-exports everything needed
