...
describe('GetAllCardDirect', async () => {
  ...
  test('direct-list-get_all_card', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-get_all_card', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DIGIMON_TCG_TEST_GET_ALL_CARD_ENTID']
