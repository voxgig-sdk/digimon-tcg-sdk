...
//     (\`new DigimonTcgSDK(...)\` / \`DigimonTcgSDK.test(...)\` become
//     \`DigimonTcgSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_all_card":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DigimonTcgSDK(...)\` and any
// \`DigimonTcgSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DigimonTcgSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DigimonTcgSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
